...
- `DATABASE_URL` is the only database secret. Never `VITE_DATABASE_URL` or `PUBFLOW_PUBLIC_DATABASE_URL`. Hosts (Neon, Coolify, Wrangler) already expect that name. Put the Neon/PlanetScale/Turso string in `DATABASE_URL` — see [Env](./env.md) and [Database](./db.md).
