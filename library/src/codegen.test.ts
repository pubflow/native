...
describe('generateNative', () => {
  it('emits $id for [id].tsx, withParams, and INDEX_HTML on the router', () => {
    ...
    expect(client).toContain("import './process-shim.ts'")
    expect(client).toContain('router.load()')
    const shim = fs.readFileSync(path.join(root, '.pubflow', 'generated', 'process-shim.ts'), 'utf8')
    expect(shim).toContain('process.env')
